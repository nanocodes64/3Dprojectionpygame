...
while True:
  ...
  rotation_x = np.matrix([[cos(angle), 0, sin(angle)], [0, 1, 0],
                          [-sin(angle), 0, cos(angle)]])
  rotation_y = np.matrix([[1, 0, 0], [0, cos(angle), -sin(angle)],
                          [0, sin(angle), cos(angle)]])

  angle += ANGLE_INCREMENT_SPEED
  ...
  for point in points:
    ...
    x = int(projected2d[0][0] * scale) + circle_position[0]
    y = int(projected2d[1][0] * scale) + circle_position[1]
